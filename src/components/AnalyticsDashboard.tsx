...
const AnalyticsDashboard = ({ avgVisitorsPerDay, amtVistiorsToday, timeseriesPageviews, topCountries }: AnalyticsDashboardProps) => {
    return (
        <div className=" flex flex-col gap-6">
            <div className="grid w-full mx-auto grid-cols-1 sm:grid-cols-2 gap-6 px-3 sm:px-5 lg:px-10">
                <Card className=" w-full mx-auto lg:max-w-xs">
                ...
                <Card className='w-full'>
                    <p className='flex gap-2.5 items-center text-tremor-default text-dark-tremor-content'>
                        ...
                        <Badge
                            percentage={
                                (amtVistiorsToday / Number(avgVisitorsPerDay) - 1) * 100
                            }
                        />
                    </p>
                    <p className='text-3xl text-dark-tremor-content-strong font-semibold'>
                ...
                <Card className="flex flex-col sm:grid grid-cols-4 gap-6">
                    <h2 className=" w-full text-dark-tremor-content-strong text-clip sm:left-left font-semibold text-xl">
                        This weeks top visitors:
                    </h2>
                    <div className=" col-span-3 flex items-center justify-between flex-wrap gap-8">
                        {topCountries?.map(([countryCode, number]: any) => {
                            return (
